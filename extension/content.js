...
const MESSAGES = {
  ...
  en: {
    ...
    // Validation Messages
    validationNoDomains: '⚠️ Warning: You haven\'t entered any domains!\n\nPlease enter at least one domain and click Continue again.',
    validationNoIPs: '⚠️ Warning: You haven\'t entered any IP addresses!\n\nPlease enter at least one IP address and click Continue again.',
    validationNoURLs: '⚠️ Warning: You haven\'t entered any URLs!\n\nPlease enter at least one URL and click Continue again.',
    validationNoRuleName: '⚠️ Warning: You haven\'t entered a rule name!\n\nPlease enter a rule name and click Continue again.',
    validationNoHeaderName: '⚠️ Warning: You haven\'t entered a header name!\n\nPlease enter the header name (e.g., X-MS-Exchange-Organization-BypassClutter) and click Continue again.',
    validationNoHeaderValue: '⚠️ Warning: You haven\'t entered a header value!\n\nPlease enter "1" and click Continue again.',
    // Workflow names
    workflow1Name: 'Third-Party Phishing Simulations',
...
const WORKFLOW_STEPS = [
  ...
  },
  {
    ...
    criticalStep: true,
    waitAfterClick: 500,
    showIPList: true,     // Bu adımda IP listesini görsel olarak göster
    hideIPList: false
  },
  {
    id: 10,
...
class FloatingPanel {
  ...
  attachEventListeners() {
    ...
    this.header.addEventListener('mousedown', (e) => {
      ...
      // Sadece sol mouse button ile sürükleme başlat
      ...
      // Mouse button'ı takip et
      this.dragButtonPressed = true
      
      e.preventDefault() // Text selection'ı engelle
    })
    ...
    const handleMouseMove = (e) => {
      ...
      // Mouse button basılı değilse sürüklemeyi durdur
    ...
    // Mouse up handler - capture phase'de dinle
    ...
    // Mouse leave handler - panel dışına çıkınca sürüklemeyi durdur
    if (this.container) {
      this.container.addEventListener('mouseleave', (e) => {
        // Eğer mouse panel dışına çıktıysa ve sürükleme aktifse
        if (this.isDragging && !this.container.contains(e.relatedTarget)) {
          // Mouse button hala basılı mı kontrol et
          // Eğer basılı değilse sürüklemeyi durdur
          setTimeout(() => {
            if (this.isDragging) {
              this.stopDragging()
            }
    ...
    // Global mouse up - her yerde mouse button bırakıldığında
...
class KeepnetAssistant {
  constructor() {
    ...
    this.currentStepConfig = null     // Aktif step config (tooltip pozisyonu için)
  }
  ...
  async executeStep(stepNum, customSteps = null) {
    try {
      ...
      // Aktif step config'i sakla (özelleştirilmiş davranışlar için)
      this.currentStepConfig = step
      ...
      const timerInterval = window.setInterval(() => {
        ...
        if (remainingTime <= 0) {
          ...
          // Workflow'a göre davranış:
          // - WORKFLOW_6: Sadece uyarı göster, kullanıcı Continue'ya basana kadar adımı değiştirme
          // - Diğer workflow'lar: önceki davranışla uyumlu olarak otomatik sonraki adıma geç
          if (this.workflowName === 'WORKFLOW_6') {
            console.log(`[Keepnet] Manual timer finished for WORKFLOW_6 step ${step.id}, waiting for user action (no auto-advance)`)
            if (this.panel && this.panel.showSuccess) {
              this.panel.showSuccess(i18n('timeUpReady'))
            }
          } else {
      ...
      if (step.target) {
        ...
        if (element) {
          ...
          if (step.findCheckboxFromRow) {
            ...
            // Satırın parent container'ını bul (DetailsRow)
            let rowContainer = element.closest('div[data-automationid="DetailsRow"]') ||
                              element.closest('div.ms-DetailsRow') ||
          ...
          // WORKFLOW 4-5 için otomatik geçiş - AMA manualTimer varsa veya autoAdvance false ise ÇALIŞMA!
          // WORKFLOW 1 Step 8-9-10 için auto-advance YAPMA (manuel Continue butonu gerekli)
          // NOT: WORKFLOW_6 için auto-advance timer devre dışı (sadece kullanıcı tıklamasıyla ilerler)
          if ((this.workflowName === 'WORKFLOW_4' || this.workflowName === 'WORKFLOW_5') 
              && !step.isSummary 
              && !step.isInfoCard 
              && !step.manualTimer
              && step.autoAdvance !== false) {  // autoAdvance false ise auto-advance YAPMA!
            const autoDelay = step.autoAdvanceDelay || 12000  // 7000 → 12000 (default değer daha yavaş)
...
${i18n(step.licenseCheck.skipMessage)}`)
  ...
  renderStepContent(step) {
    ...
    if (step.id === 8 && step.name === 'step8_domains_input' && this.workflowName === 'WORKFLOW_1') {
      const domains = [
  ...
  highlightElement(element, tooltipText) {
    ...
    // Tooltip - elementin ÜSTÜNDE, elementi KAPATMADAN göster
    if (tooltipText) {
      ...
      // Varsayılan: elementin ÜSTÜNDE ve ortalanmış konum (elementi kapatmayacak şekilde)
      const centerX = rect.left + (rect.width / 2) - (tooltipWidth / 2)
      let verticalGap = rect.height + margin
      
      // Belirli adımlar için (WF4, WF5, WF6 belirtilen step'ler) ekstra boşluk bırak
      const step = this.currentStepConfig
      if (step && (
        (this.workflowName === 'WORKFLOW_4' && step.id === 10 && step.name === 'spambypass_step10_do_following_dropdown') ||
        (this.workflowName === 'WORKFLOW_5' && step.id === 9 && step.name === 'spambypass_step10_do_following_dropdown') ||
        (this.workflowName === 'WORKFLOW_6' && step.id === 9 && step.name === 'atpattach_step9_do_following')
      )) {
        verticalGap = rect.height * 2 + 40  // Bu adımlarda tooltip'i daha da yukarı al
      }
      
      const aboveY = rect.top - tooltipHeight - verticalGap
      
      // Yatayda ve dikeyde ekran sınırlarına clamp et
      const finalX = Math.max(10, Math.min(centerX, window.innerWidth - tooltipWidth - 10))
      const finalY = Math.max(10, aboveY)
  ...
  async nextStep() {
    ...
    if (this.workflowName === 'WORKFLOW_1' && currentStepConfig) {
      ...
      if (stepName === 'step9_ip_input') {
        ...
        const root = document.querySelector('.ms-Panel-main, [role="dialog"]') || document.body
        if (root) {
          // IP picker içindeki chip/tag'leri kontrol et - daha geniş selector
          const ipChips = root.querySelectorAll(
            '.ms-BasePicker-text .ms-PickerPersona-container, ' +
            '.ms-TagItem, ' +
            '.ms-Persona-primaryText, ' +
            '[data-automation-id="FieldRendererAnchor-SenderIpRanges"] .ms-BasePicker-text > div, ' +
            '[data-automation-id="SenderIpRanges"] .ms-BasePicker-text > div'
          )
          console.log('[Keepnet] IP chips found:', ipChips.length)
          
          // Alternatif: Text content kontrolü
          const ipText = root.textContent || root.innerText || ''
          // Genel IP deseni: 0-255 arası kontrol etmiyoruz, sadece format
          const hasIPsFromText = /(\d{1,3}\.){3}\d{1,3}/.test(ipText)
          const hasIPs = hasIPsFromText || ipChips.length > 0
    ...
    if (this.workflowName === 'WORKFLOW_4' && currentStepConfig) {
      ...
      if (stepName === 'spambypass_step9_enter_ip') {
        ...
        // Bazı tenant UI'larında IP'ler TagItem yerine farklı chip bileşenleriyle gösteriliyor.
        // Bu yüzden Workflow 1'deki IP kontrolü ile aynı geniş selector mantığını kullanıyoruz.

        // 1) Panel içindeki tüm olası IP chip/tag elementlerini bul
        const root = document.querySelector('.ms-Panel-main, [role="dialog"]') || document.body
        if (root) {
          const ipChips = root.querySelectorAll(
            '.ms-BasePicker-text .ms-PickerPersona-container, ' +
          ...
          console.log('[Keepnet] IP chips (WF4) found:', ipChips.length)

          // 2) Alternatif: Panel text içinden IP pattern'i ara (ör. 149.72.x.x)
          const ipText = root.textContent || root.innerText || ''
          const hasIPsFromText = /(\d{1,3}\.){3}\d{1,3}/.test(ipText)

          const hasIPs = ipChips.length > 0 || hasIPsFromText
